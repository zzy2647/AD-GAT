...
class VAN_b1(nn.Module):
    def __init__(self,num_classes=3,**kwargs):
        super(VAN_b1, self).__init__()
        self.van_b1 = van_b1(in_chans=1, num_classes = num_classes)

    def forward(self,x):
        return self.van_b1(x)     

class VAN_b0(nn.Module):
    def __init__(self,num_classes=3,**kwargs):
        super(VAN_b0, self).__init__()
        self.van_b0 = van_b0(in_chans=1, num_classes = num_classes)

    def forward(self,x):
        return self.van_b0(x)     
if __name__ == '__main__':
    x = torch.randn((4, 1, 224, 224))
    model = van_b1(in_chans=1, num_classes=3)
    print(model)
    y = model(x)
    print(y.shape)
