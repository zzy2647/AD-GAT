...
from Models.layers.anti_aliasing import AntiAliasDownsampleLayer
from Models.layers.avg_pool import FastGlobalAvgPool2d
from Models.layers.squeeze_and_excite import SEModule, SKConv
from Models.layers.space_to_depth import SpaceToDepthModule
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, use_se=True, anti_alias_layer=None):
        ...
        self.sk = SKConv(planes * self.expansion, planes * self.expansion) if use_se else None
    def forward(self, x):
        ...
        if self.sk is not None: out = self.sk(out)
...
def TResnetM(num_classes = 3, remove_aa_jit = False):
...
print(device)
inputs = torch.randn(4, 1, 224, 224).to(device)

model = TResnetM(num_classes = 3).to(device)
print(model)
