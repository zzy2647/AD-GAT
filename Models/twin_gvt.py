...
class CPVTV2(PyramidVisionTransformer):
    ...
    """
    def __init__(self, img_size=224, patch_size=4, in_chans=1, num_classes=3, embed_dims=[64, 128, 256, 512],
                 num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], block_cls=Block):
...
class ALTGVT(PCPVT):
    ...
    alias Twins-SVT
    """
    def __init__(self, img_size=224, patch_size=4, in_chans=1, num_classes=3, embed_dims=[64, 128, 256],
                 num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
...
@register_model
def pcpvt_small_v0(pretrained=False, num_classes = 3, **kwargs):
    model = CPVTV2(
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], in_chans=1, num_classes=num_classes,  mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
